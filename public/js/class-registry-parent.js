(() => {
  ...
  function openVideo(item) {
    ...
    if (!videoUrl && item.youtubeVideoId) {
      videoUrl = `https://www.youtube.com/embed/${item.youtubeVideoId}?rel=0&playsinline=1`;
    }
    ...
    const sidebarTitle = $("lesson-video-sidebar-title");
    if (sidebarTitle) {
      sidebarTitle.textContent = subjectLabels[item.subject] || "مشاهدة الحصة";
    }
    const sidebarMeta = $("lesson-video-sidebar-meta");
    if (sidebarMeta) {
      sidebarMeta.textContent = `${formatDate(item.scheduledAt)} · مشاهدة داخل المنصة`;
    }
